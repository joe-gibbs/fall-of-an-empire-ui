...
export function useLedgerOverviewBridge(activeTab: string, rowOffset = 0, rowLimit = 0): GetLedgerOverviewResponse | null {
  const cacheKey = `${activeTab}:${rowOffset}:${rowLimit}`;
  const live = useBridgeQuery({
    action: 'game.get_ledger_overview',
    payload: { activeTab, rowOffset, rowLimit },
