...
import { applyFormationTemplateBridge } from '../../../bridge/military-map/useFormationTemplatesBridge';
              ...
              return (
                <Tooltip key={item.id} content={content} position="left" delay={150}>
                  <div className="mil-queue-next-item">
                    <img src={item.portrait} alt="" className="mil-queue-next-icon" />
...
function FormationCard({
  f, unitByKey, blockedReason, onRaise,
}: {
  f: FormationTemplate;
  unitByKey: Map<string, RecruitableUnit>;
  ...
  return (
    <Tooltip content={tooltip} position="left" delay={200}>
      <div className={`mil-formation${locked ? ' mil-formation--locked' : ''}`}>
        <div className="mil-formation-header">
          <span className="mil-formation-title">
            <img src={iconProfile.icon} alt="" className="mil-formation-icon" />
        ...
        <div className="mil-formation-body">
          ...
          <div className="mil-formation-meta">
            <span className="mil-formation-meta-item">
              <img src="/assets/icons/I_Swords.png" alt="" className="mil-meta-icon" />
              {n(f.totalStrength)}
            </span>
            <span className="mil-formation-meta-item">
              <img src="/assets/icons/I_Coins.png" alt="" className="mil-meta-icon" />
              {n(f.totalPrice)}
            </span>
        ...
        <div className="mil-formation-action">
          {locked ? (
            <div className="mil-formation-locked">
              <img src="/assets/icons/I_Locked.png" alt="" className="mil-locked-icon" />
              <span>{lockReasons[0] ?? webUIText("Auto.Fix.ExprFallback.componentssidebarsSettlementMilitaryPanel.221.1")}</span>
            </div>
          ) : (
            <button type="button" className="mil-raise-btn" onMouseDown={() => onRaise(f.id)}>
  ...
  return (
    <Tooltip content={content} position="bottom" delay={150}>
      <span className="mil-formation-slot">
        <img src={slot.portrait} alt="" className="mil-formation-slot-portrait" />
...
const SettlementMilitaryPanel: React.FC<Props> = ({ settlement }) => {
  ...
  if (!r) {
    return <div className="sidebar-placeholder"><WebUIText textKey="Auto.ComponentsSidebarsSettlementMilitaryPanel.299.6" /></div>;
  }
  ...
  return (
    <div className={`mil-panel${blockedReason ? ' mil-panel--blocked' : ''}`}>
      ...
      <TrainingQueue items={r.trainingQueue} unitByKey={unitByKey} />
          ...
          return (
            <div key={type} className={`mil-cap${locked ? ' mil-cap--locked' : ''}`}>
              <Tooltip content={rowTooltip} position="left" delay={200}>
                <img src={TYPE_ICONS[type]} alt="" className="mil-cap-icon" />
              </Tooltip>
              <div className="mil-cap-body">
                ...
                {locked ? (
                  <span className="mil-cap-locked-text">
                    <img src="/assets/icons/I_Locked.png" alt="" className="mil-cap-lock" />
                    ...
                    {available.map(u => (
                      ...
                      >
                        <span className="mil-cap-unit">
                          <img src={u.portrait} alt="" className="mil-cap-portrait" />
                        </span>
                      </Tooltip>
                    ))}
                  </div>
              ...
              <div className="mil-cap-tier">
                {locked ? (
                  <span className="mil-cap-tier-empty">
                    <img src="/assets/icons/I_Minus.png" alt="" className="mil-cap-tier-empty-icon" draggable={false} />
                  </span>
                ) : (
                  <>
